...
def weekly_get(request: Request, week: str | None = None):
    ...
    with connect() as conn:
        ...
        agg = conn.execute(
            """SELECT
                 ...
                 SUM(CASE WHEN caffeine  = 1 THEN 1 ELSE 0 END) AS caffeine_days,
                 SUM(CASE WHEN alcohol   = 1 THEN 1 ELSE 0 END) AS alcohol_days,
                 SUM(CASE WHEN late_meal = 1 THEN 1 ELSE 0 END) AS late_meal_days,
    ...
    return templates.TemplateResponse(request, "weekly.html", {
        ...
        "caffeine_days":  agg["caffeine_days"]  or 0,
        "alcohol_days":   agg["alcohol_days"]   or 0,
        "late_meal_days": agg["late_meal_days"] or 0,
